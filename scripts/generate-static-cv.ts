...
function buildCV() {
  ...
  const contactText = 'Email: educationpoint0360@gmail.com  |  Phone / WhatsApp: +92 331 4220506  |  Mianwali, Pakistan';
  doc.text(contactText, margin, y);
  y += 13;

  const linksText = 'Portfolio: https://rizwanep.netlify.app/  |  Flagship Platform: https://educationpoint360.netlify.app/';
  ...
  // Section: FOUNDER EXPERIENCE & INDEPENDENT PROJECTS
  ...
  // Section: FEATURED DIGITAL PROJECTS
  ...
  // Section: CERTIFICATIONS & LANGUAGES & GLOBAL AVAILABILITY
  ...
  doc.text('Curriculum Vitae — Muhammad Rizwan  |  https://rizwanep.netlify.app/  |  Think Digital. Build Future.', margin, doc.internal.pageSize.getHeight() - 20);

  // Ensure output directory exists
  const publicDir = path.resolve('public/cv');
  if (!fs.existsSync(publicDir)) {
    fs.mkdirSync(publicDir, { recursive: true });
  }

  const outPath = path.join(publicDir, 'Muhammad-Rizwan-CV.pdf');
  const buffer = Buffer.from(doc.output('arraybuffer'));
  fs.writeFileSync(outPath, buffer);
  console.log(`Generated ${outPath} successfully (${buffer.length} bytes).`);
}
